...
Example of concatenated depth file (NOTE: The Sample_Depth names may need to be edited by the user to match the Sample names in the reads_file.tsv):

```{bash, eval=FALSE}
contigName              contigLen   totalAvgDepth         Sample_Depth         Depth
S4B_scaffold_41967_c1   2430.0      15.7917               SF_May12_USGS_4_1    15.7917
S4B_scaffold_22085_c1   3249.0      12.081                SF_May12_USGS_4_1    12.081
S4B_scaffold_39665_c1   2489.0      9.998289999999999     SF_May12_USGS_4_1    9.998289999999999
S4B_scaffold_36461_c1   2583.0      8.70695               SF_May12_USGS_4_1    8.70695
S4B_scaffold_62858_c1   2036.0      11.1448               SF_May12_USGS_4_1    11.1448
S4B_scaffold_47653_c1   2298.0      10.1196               SF_May12_USGS_4_1    10.1196
S4B_scaffold_28262_c1   2900.0      8.95927               SF_May12_USGS_4_1    8.95927
```
...
```{bash, eval=FALSE}
python3 files_prep.py -b bin2sample.tsv -f ~/depth_files/ -q ~/fastq_files/ -a ~/fna_files/
```
...
1.1. The Sample column un the reads file and the Sample_Depth column in the depth file must contain matching sample names. If this is not currently formatted correctly, manually adjust the names of each column in their respective file. For an example, please look at the `example__reads.tsv` and `example__depth.tsv` files.
...
usage: bin_abundance_viz.py [-h] -b BIN_ABUNDANCE -t TAXONOMY_INFO
                            [-s SAMPLE2SITE] [-p PERCENT] [-w WIDTH]
                            [-l HEIGHT] [-d DPI] [-o OUT_FIG] [-c TAXA_COLOR]

optional arguments:
  -h, --help            show this help message and exit
  -b BIN_ABUNDANCE, --bin_abundance BIN_ABUNDANCE
                        Input file outputted from the bin_abundance.py script
                        with file extension.
  -t TAXONOMY_INFO, --taxonomy_info TAXONOMY_INFO
                        Input file mapping taxonomy to each bin with file
                        extension.
  -s SAMPLE2SITE, --sample2site SAMPLE2SITE
                        Input file containing each sample to the site it was
                        taken from with file extension [""].
  ...
  -c TAXA_COLOR, --taxa_color TAXA_COLOR
                        Input file containing the RGB color code for each taxa
                        with file extension [""].
```  
...
Example of sample2site file - the Sample names should be the unique names in the Sample column of the `bin_abundance.py` output; the Site names are the physical location's name/code that each sample was taken from:
...
## Mapping sample to site

The user must manually map each sample to its respective site that the sample was extracted from.
